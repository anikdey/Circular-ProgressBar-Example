...
class CircularProgressBar : View {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        attrs?.let {attributes->
            var typedArray = context.obtainStyledAttributes(attributes, R.styleable.CircularProgressBar)
            textSize = typedArray.getDimensionPixelSize(R.styleable.CircularProgressBar_mtextSize, textSize)
            startAngel = typedArray.getFloat(R.styleable.CircularProgressBar_mstartAngle, startAngel)
            progressBarWidth = typedArray.getFloat(R.styleable.CircularProgressBar_progressBarWidth, progressBarWidth)
            percentCompleted = typedArray.getFloat(R.styleable.CircularProgressBar_mpercentCompleted, percentCompleted)
            innerCircleColor = typedArray.getColor(R.styleable.CircularProgressBar_innerCircleColor, innerCircleColor)
            textColor = typedArray.getColor(R.styleable.CircularProgressBar_mtextColor, textColor)
            progressCircleColor = typedArray.getColor(R.styleable.CircularProgressBar_progressCircleColor, progressCircleColor)
            typedArray.recycle()
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        var innerGap = 20f + (progressBarWidth/2)
        innerRect = RectF(progressBarWidth+innerGap, progressBarWidth+innerGap, calculatedWidth.toFloat() - progressBarWidth-innerGap, calculatedHeight.toFloat()- progressBarWidth-innerGap)

        progressRect = RectF(progressBarWidth, progressBarWidth, calculatedWidth.toFloat()-progressBarWidth, calculatedHeight.toFloat()-progressBarWidth)
    ...
    inner class CircleAngleAnimation(private val circle: CircularProgressBar, percentCompleted: Float) : Animation() {
